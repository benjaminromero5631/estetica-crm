...
const sede = 'puerto-montt'
const servicio = 'otomodelacion'

// ── component ────────────────────────────────────────────────
function ReservaDirectaInner() {
  ...
  async function continuarConLead(e: React.FormEvent) {
    e.preventDefault()
    ...
    try {
      const res = await fetch('/api/public/leads-directo', {
        method: 'POST',
      ...
      setLeadError(`Error inesperado: ${err instanceof Error ? err.message : JSON.stringify(err)}`)
      setSubmittingLead(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 text-white flex flex-col items-center py-10 px-4">
        <div className="w-full max-w-md mb-8 text-center">
          <h1 className="text-2xl font-semibold tracking-tight">Reserva tu evaluación</h1>
          <p className="text-zinc-400 text-sm mt-1">Cuéntanos quién eres para continuar</p>
        </div>

        <form onSubmit={continuarConLead} className="w-full max-w-md bg-zinc-900 rounded-2xl p-5 border border-zinc-800 space-y-4">
          <div>
            <label className="block text-xs text-zinc-400 uppercase tracking-wide font-medium mb-2">Nombre</label>
            <input
              type="text"
              value={nombre}
              onChange={e => setNombre(e.target.value)}
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500"
              placeholder="Tu nombre"
            />
          </div>
          <div>
            <label className="block text-xs text-zinc-400 uppercase tracking-wide font-medium mb-2">Teléfono</label>
            <input
              type="tel"
              value={telefono}
          ...
          <button
            type="submit"
            disabled={submittingLead || !nombre.trim() || !telefono.trim()}
          ...
          >
            {submittingLead ? 'Continuando...' : 'Continuar'}
          </button>
        </form>
      </div>
    )
...
export default function ReservaDirectaPage() {
  return (
